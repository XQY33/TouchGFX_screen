...
#define SCCB_SCL_CLK_ENABLE       __HAL_RCC_GPIOI_CLK_ENABLE()		// SCL 引脚时钟
#define SCCB_SCL_PORT   		   GPIOI                 				// SCL 引脚端口
#define SCCB_SCL_PIN     		   GPIO_PIN_8  							// SCL 引脚

#define SCCB_SDA_CLK_ENABLE       __HAL_RCC_GPIOI_CLK_ENABLE() 	// SDA 引脚时钟
#define SCCB_SDA_PORT   			 GPIOI                   			// SDA 引脚端口
...
uint8_t  SCCB_WriteReg (uint8_t addr,uint8_t value);     // 对指定的寄存器写一字节数据
uint8_t  SCCB_ReadReg (uint8_t addr);                    // 对指定的寄存器读取一字节数据
...
#endif /* APPLICATION_USER_HARDWARE_SCCB_H_ */
